...
class RegistrationCubit extends Cubit<RegistrationState> {
  ...
  void setSelectedType(int? userTypeId) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(userTypeId: userTypeId)));
  }
  ...
  void setName(String? name) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(name: name)));
  }

  void setUserName(String? userName) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(userName: userName)));
  }

  void setPhoneNumber(String? phoneNumber) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(phoneNumber: phoneNumber)));
  }

  void setEmail(String? email) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(email: email)));
  }

  void setNationalId(String? nationalId) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(nationalId: nationalId)));
  }

  void setDateOfBirth(DateTime? date, BuildContext context) {
    state.dobController.text =
        AppDateTimeFormat.convertSentDate(date, context) ?? "";
    emit(
        state.copyWith(registerModel: state.registerModel.copyWith(dob: date)));
  }

  void setGender(Gender? gender) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(genderId: gender?.id())));
  }

  void setMaritalStatus(Item? status) {
    emit(state.copyWith(registerModel: state.registerModel.copyWith(
  ...
  Future<void> initThird() async {
    ...
    if (countries != null && companies != null) {
      emit(state.copyWith(registrationStatus: SecondSuccess(),
          companies: companies,
          countries: countries));
    } else {
  ...
  void setCountry(Country? country) {
    emit(state.copyWith(registerModel: state.registerModel.copyWith(
        addressDto: state.registerModel.addressDto?.copyWith(
  ...
  void setPassword(String? password) {
    emit(state.copyWith(
        registerModel: state.registerModel.copyWith(password: password)));
  }

  /// registration request
  Future<bool> register() async {
    await EasyLoading.show(indicator: CircularProgressIndicator());
    emit(state.copyWith(registrationStatus: RegisterLoading()));

    var result = await repository.register(
      userDto: state.registerModel,
    );
    await EasyLoading.dismiss();
    return result.fold(
          (failure) {
        emit(state.copyWith(registrationStatus: RegisterFailure()));
        return false;
      },
          (result) {
        emit(state.copyWith(registrationStatus: RegisterSuccess()));
        return true;
      },
    );
  }
